...
class AnthropicProvider(LLMProviderInterface):
    ...
    async def generate_exercise(
        ...
        try:
            response = await self.client.messages.create(
                model=self.model,
                system=EXERCISE_GENERATION_SYSTEM,
            ...
            logger.error("Anthropic returned non-JSON for exercise generation: %s", exc)
            raise LLMProviderError("anthropic", "Invalid JSON in exercise generation response.")
        except Exception as exc:
            logger.error("Anthropic exercise generation failed: %s", exc, exc_info=True)
    ...
    async def evaluate_answer(
        ...
        try:
            response = await self.client.messages.create(
                model=self.model,
                system=EXERCISE_EVALUATION_SYSTEM,
                messages=[{"role": "user", "content": user_msg}],
                max_tokens=self.max_tokens,
                temperature=0.3,
            )
            ...
            logger.error("Anthropic returned non-JSON for evaluation: %s", exc)
            raise LLMProviderError("anthropic", "Invalid JSON in evaluation response.")
        except Exception as exc:
            logger.error("Anthropic evaluation failed: %s", exc, exc_info=True)
            raise LLMProviderError("anthropic", str(exc))
